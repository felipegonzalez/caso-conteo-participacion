...
generated quantities {
  ...
  real sigma = fabs(normal_rng(0, sigma_pars));
  real y[n];

  for(i in 1:num_estratos) {
    p_est[i] = beta_proportion_rng(mu, kappa);
    log_sigma_est[i] = normal_rng(0, sigma);
  }
  for(i in 1:n){
    int j = muestra[i];
    real sigma_est = exp(log_sigma_est[est[j]]);
    real p_casilla = normal_rng(p_est[est[j]], sigma_est * sqrt(p_est[est[j]]*(1-p_est[est[j]]) / nom[j]));
    y[i] = nom[j] * p_casilla;
